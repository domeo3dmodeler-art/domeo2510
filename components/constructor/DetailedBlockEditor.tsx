...
const DetailedBlockEditor: React.FC<DetailedBlockEditorProps> = ({
    ...
    const loadAvailableProperties = async () => {
      if (editedBlock.catalogCategoryId) {
        try {
          ...
          clientLogger.error('Error loading available properties', error instanceof Error ? error : new Error(String(error)));
        }
      } else {
  ...
  const loadPropertyValues = async (propertyKey: string) => {
    ...
    clientLogger.debug('Loading property values', { propertyKey, categoryId: editedBlock.catalogCategoryId });
    ...
    try {
      ...
      clientLogger.debug('Response status', { status: response.status });
      ...
      if (response.ok) {
        ...
        if (data.success && data.data.products) {
          clientLogger.debug('Products count', { count: data.data.products.length });
            ...
            if (firstProduct.properties_data) {
              ...
              if (typeof propertiesData === 'string') {
                try {
                  ...
                  clientLogger.warn('Failed to parse properties_data for logging', { error: e instanceof Error ? e.message : String(e) });
                }
              }
              
              clientLogger.debug('Available property keys', { keys: Object.keys(propertiesData), lookingFor: propertyKey });
            }
          }
            ...
            if (product.properties_data) {
              ...
              if (typeof propertiesData === 'string') {
                try {
                  ...
                  clientLogger.warn('Failed to parse properties_data', { error: e instanceof Error ? e.message : String(e) });
                  return;
                }
              }
              ...
              if (!foundValue) {
                ...
                if (matchingKey) {
                  ...
                  clientLogger.debug('Found property by partial match', { matchingKey, propertyKey });
                }
              }
          ...
          clientLogger.debug('Unique values found', { values: Array.from(values), valueCounts: Object.fromEntries(valueCounts) });
        ...
        clientLogger.error('API request failed', { status: response.status, statusText: response.statusText });
      }
    } catch (error) {
      clientLogger.error('Error loading property values', error instanceof Error ? error : new Error(String(error)));
    }
  };
