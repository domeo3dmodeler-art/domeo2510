...
export default function ConfiguratorExportSettingsPage() {
  ...
  useEffect(() => {
    if (selectedCatalogCategory) {
      loadExportConfigs(selectedCatalogCategory);
    }
  ...
  const loadExportConfigs = async (categoryId: string) => {
    try {
      const response = await fetch(`/api/catalog/export/config?catalogCategoryId=${categoryId}`);
  ...
  const handleExport = async (exportType: 'quote' | 'invoice' | 'supplier_order') => {
    ...
    try {
      const response = await fetch('/api/catalog/export', {
        ...
        body: JSON.stringify({
          catalogCategoryId: selectedCatalogCategory,
          exportType
        })
      });
  ...
  return (
    <div className="space-y-6">
      {/* Выбор категории */}
      <Card className="p-4">
        <h3 className="text-lg font-semibold mb-4">Выбор категории для настройки экспорта</h3>
        ...
        {selectedFrontendCategory && (
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-700">
              <strong>Примечание:</strong> Настройки экспорта будут применяться к товарам из выбранной категории каталога 
              при экспорте через категорию конфигуратора "{frontendCategories.find(c => c.id === selectedFrontendCategory)?.name}".
            </p>
          </div>
        )}
      </Card>
