    ...
    if (status === 'PAID' && !wasPaid) {
      try {
        ...
        if (existingOrder) {
          ...
          // DEPRECATED: Автоматическое создание Order при оплате Invoice больше не выполняется
          // Order теперь создается первым из корзины, Invoice создается на основе Order
        }
      } catch (orderError) {
