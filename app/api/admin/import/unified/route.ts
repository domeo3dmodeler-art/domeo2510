    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        ...
        if (nameField && properties[nameField]) {
          ...
          // Если название не найдено в файле - устанавливаем по умолчанию
          // Для режима обновления название можно будет взять из БД, если товар существует
        ...
        if (!internalSku || internalSku.trim() === '') {
          ...
          // В режиме обновления название НЕ обязательно в файле
          // Название можно взять из существующего товара в БД, если его нет в файле
          // Если товар не найден в БД - будет ошибка позже
          // Поэтому здесь НЕ проверяем наличие названия
          console.log(`  ℹ️ Режим обновления: название "${product.name}" (если товар не найден в БД, название будет взято из БД или будет ошибка)`);
        }
