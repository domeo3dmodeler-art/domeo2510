  ...
  try {
    ...
    if (uncachedModels.length > 0) {
      ...
      for (const [modelName, propertyValue] of modelToValue.entries()) {
        ...
        // Если не найдено по артикулу, ищем по "Domeo_Название модели для Web"
        if (propertyPhotos.length === 0) {
          console.log(`🔍 Фото не найдено по артикулу для "${modelName}", пробуем поиск по названию модели`);
